...
{id:'core',title:'Verze, manifest a identita',tests:[
 ['manifest',async()=>{const j=await json('../studio-manifest.json');return j.id==='sortio'&&j.version==='1.1.13'?'Manifest SORTIO 1.1.13 je platný.':false}],
 ...
 ['ownership',async()=>appText().then(t=>t.includes('Daniel Baláž')&&t.includes('Gymnázium, Ostrava-Hrabůvka')?'Autorství a školní projekt jsou uvedeny.':false)],
 ['pwa-manifest',async()=>{const j=await json('../manifest.webmanifest');return j.short_name==='SORTIO'&&j.icons.length>=2&&j.icons.some(icon=>String(icon.purpose||'').includes('maskable'))?'PWA manifest obsahuje název a ikony.':false}],
 ['service-worker',async()=>text('../sw.js').then(t=>t.includes('ghrab-sortio-v1.1.13')&&t.includes('CACHE_PREFIXES')&&t.includes('CACHE_PREFIXES.some')&&t.includes('key !== CACHE_NAME')?'Service worker má verzi 1.1.13 a izolaci cache.':false)]
]},
{id:'data',title:'Datový trezor, migrace a zálohy',tests:[
 ['schema-v5',async()=>appText().then(t=>t.includes("DATA_KEY='sortio.data.v5'")&&t.includes("schema:'sortio-data-v5'")?'Datový model v5 je aktivní.':false)],
 ['migration',async()=>appText().then(t=>t.includes("'sortio.data.v4','sortio.data.v3','sortio.data.v2'")?'Migrace z verzí 0.2–0.4 je připravena.':false)],
 ['last-good',async()=>appText().then(t=>t.includes('LAST_GOOD_KEY')&&t.includes('persistDataSnapshot')?'Poslední bezpečný stav se udržuje.':false)],
 ['corrupt-recovery',async()=>appText().then(t=>t.includes('CORRUPT_KEY')&&t.includes('data-primary-corrupt')?'Poškozený primární zápis má záchrannou cestu.':false)],
 ['backup-checksum',async()=>appText().then(t=>t.includes('checksumText')&&t.includes('validateBackupPayload')?'Zálohy mají kontrolní součet a validaci.':false)],
 ['backup-limit',async()=>appText().then(t=>t.includes('MAX_BACKUP_BYTES')&&t.includes('5 MB')?'Import omezuje velikost souboru.':false)],
 ['pre-import-recovery',async()=>appText().then(t=>t.includes('RECOVERY_KEY')&&t.includes('restoreRecoverySnapshot')?'Stav před importem lze obnovit.':false)]
]},
{id:'import',title:'Import z interního systému',tests:[
 ...
 ['email-not-stored',async()=>appText().then(t=>t.includes('emailDataStored:false')&&t.includes('Importované e-mailové adresy se po vytvoření náhledu neukládají')?'Rozhraní i diagnostika deklarují neukládání e-mailů.':false)]
]},
{id:'organisation',title:'Losování, skupiny, role a místa',tests:[
 ...
 ['draw',async()=>appText().then(t=>t.includes('performDraw')&&t.includes('undoLastDraw')?'Losování a vrácení tahu jsou aktivní.':false)],
 ['hard-rules',async()=>appText().then(t=>['solveSmartPartition','validateGroupConfiguration','addPairRule','setStudentPin'].every(x=>t.includes(x))?'Závazná pravidla skupin jsou aktivní.':false)],
 ['roles',async()=>appText().then(t=>t.includes('assignRolesToAllGroups')&&t.includes('assignTopicsToGroups')?'Role a témata jsou aktivní.':false)],
 ['seating',async()=>appText().then(t=>t.includes('createSeatLayout')&&t.includes('assignSeating')&&t.includes('toggleSeatLock')?'Zasedací pořádek je aktivní.':false)]
]},
{id:'live',title:'Projekce, nástroje a výstupy',tests:[
 ['projection',async()=>appText().then(t=>t.includes('id="projectionDialog"')&&t.includes('openProjection')&&t.includes('projectionGroups')?'Bezpečný projekční režim je dostupný.':false)],
 ['projection-privacy',async()=>appText().then(t=>t.includes('Na projekci nejsou zobrazeny interní úrovně')?'Projekce má bezpečnostní upozornění.':false)],
 ['tools',async()=>appText().then(t=>['data-timer-display','data-stopwatch-display','data-score-change'].every(x=>t.includes(x))?'Třídní nástroje jsou aktivní.':false)],
 ['fair-engagement',async()=>appText().then(t=>t.includes('selectFairStudent')&&t.includes('engagementHistory')?'Spravedlivé zapojování je aktivní.':false)],
 ['print',async()=>appText().then(t=>t.includes('printSortioDocument')&&t.includes('Kartičky se jmény')?'Tiskové a PDF výstupy jsou aktivní.':false)]
]},
{id:'production',title:'Produkční provoz a přístupnost',tests:[
 ['health-console',async()=>appText().then(t=>t.includes('productionHealth')&&t.includes('runProductionChecks')?'Produkční konzole je přítomná.':false)],
...
async function text(url){const r=await fetch(url,{cache:'no-store'});if(!r.ok)throw new Error(`HTTP ${r.status}`);return r.text()}async function appText(){const[html,app,productionTools]=await Promise.all([text('../index.html'),text('../app.js'),text('../lazy/production-tools.js')]);return`${html}\n${app}\n${productionTools}`}async function json(url){return JSON.parse(await text(url))}
...
function protocol(){return JSON.stringify({schema:'sortio-runtime-tests-v5',version:'1.1.13',createdAt:new Date().toISOString(),userAgent:navigator.userAgent,results},null,2)}
