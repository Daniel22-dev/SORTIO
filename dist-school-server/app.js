...
function showDrawResult(result){const root=$('#drawResult');if(!root)return;root.classList.remove('revealed');void root.offsetWidth;const names=result.selected.map(item=>item.displayName);const multiSize=names.length<=3?36:names.length<=6?31:names.length<=10?26:22;root.innerHTML=result.selected.length===1?`<span>VYBRÁN/A</span><h2>${escapeHtml(names[0])}</h2><p>${result.exhausted?'Cyklus je dokončen. Příští losování otevře nový.':`V cyklu zbývá ${result.remaining} studentů.`}</p>`:`<span>${currentDrawMode()==='order'?'NÁHODNÉ POŘADÍ':'VYBRANÁ SKUPINA'}</span><ol class="draw-multi-names ${names.length>5?'many':''}" style="--draw-name-size:${multiSize}px">${names.map(name=>`<li>${escapeHtml(name)}</li>`).join('')}</ol><p>${result.exhausted?'Cyklus je dokončen.':`V cyklu zbývá ${result.remaining} studentů.`}</p>`;root.classList.add('revealed');refreshDrawHistory()}
...
;
function seatingCapacity(template='rows',rows=4,columns=6){rows=Math.max(2,Math.min(10,Number(rows)||4));columns=Math.max(2,Math.min(12,Number(columns)||6));if(template==='u')return columns+Math.max(0,rows-1)*2;if(template==='pairs')return rows*columns*2;return rows*columns}
function seatingCapacityText(template='rows',rows=4,columns=6){rows=Math.max(2,Math.min(10,Number(rows)||4));columns=Math.max(2,Math.min(12,Number(columns)||6));if(template==='pairs')return`${rows} řady × ${columns} dvojic × 2 místa = ${seatingCapacity(template,rows,columns)} míst`;if(template==='islands')return`${rows} ostrůvky × ${columns} míst = ${seatingCapacity(template,rows,columns)} míst`;if(template==='u')return`${columns} míst vpředu + ${Math.max(0,rows-1)} × 2 boční místa = ${seatingCapacity(template,rows,columns)} míst`;return`${rows} řady × ${columns} míst = ${seatingCapacity(template,rows,columns)} míst`}
...
;
function seatingTemplateCopy(template){if(template==='pairs')return{rows:'Počet řad (od tabule dozadu)',columns:'Dvojic lavic v jedné řadě',help:'Každá dvojice znamená dvě místa vedle sebe. Řada vede zleva doprava; řady se počítají od tabule směrem dozadu.'};if(template==='islands')return{rows:'Počet ostrůvků',columns:'Míst v jednom ostrůvku',help:'Každý ostrůvek je samostatná skupina míst. Pořadí ostrůvků v náhledu začíná u tabule.'};if(template==='u')return{rows:'Hloubka bočních ramen',columns:'Míst v přední části U',help:'Horní část U je u tabule. Další řady přidávají vždy jedno místo na levé a jedno na pravé straně.'};return{rows:'Počet řad (od tabule dozadu)',columns:'Míst v jedné řadě',help:'Jedna řada vede vodorovně zleva doprava. Řady se počítají od tabule směrem dozadu.'}}
function renderSeatingView(){const root=$('#seatingWorkspace');if(!root)return;const classItem=getSelectedClass();if(!classItem){root.innerHTML=noClassMessage('Zasedací pořádek','Nejprve importujte nebo vytvořte třídu.');return}const plan=classItem.seatingPlan;const students=classStudents(classItem);const usable=plan.seats.filter(seat=>!seat.blocked).length;const capacity=seatingCapacity(plan.template,plan.rows,plan.columns);const copy=seatingTemplateCopy(plan.template);root.innerHTML=`<div class="seating-layout"><aside class="seating-controls"><div class="assignment-heading"><span>UČEBNA</span><h2>Nastavení prostoru</h2><p>Tabule je v náhledu vždy nahoře. Po automatickém rozsazení můžete žáky ručně přetahovat mezi místy.</p></div><label>Rozložení<select id="seatingTemplate"><option value="rows" ${plan.template==='rows'?'selected':''}>Klasické řady – jednotlivá místa</option><option value="pairs" ${plan.template==='pairs'?'selected':''}>Dvojice lavic – 2 žáci vedle sebe</option><option value="islands" ${plan.template==='islands'?'selected':''}>Ostrůvky</option><option value="u" ${plan.template==='u'?'selected':''}>Uspořádání do U</option></select></label><p class="seating-template-help" id="seatingTemplateHelp">${escapeHtml(copy.help)}</p><div class="seating-dimensions"><label><span id="seatRowsLabel">${escapeHtml(copy.rows)}</span><input id="seatingRows" type="number" min="2" max="10" value="${plan.rows}"></label><label><span id="seatColumnsLabel">${escapeHtml(copy.columns)}</span><input id="seatingColumns" type="number" min="2" max="12" value="${plan.columns}"></label></div><div class="seating-capacity-note" id="seatingCapacityNote">${escapeHtml(seatingCapacityText(plan.template,plan.rows,plan.columns))}</div><button class="small-button wide-button" data-action="apply-seating-layout">Použít rozložení</button><div class="seating-stats"><div><b>${students.filter(s=>s.present).length}</b><span>přítomných</span></div><div><b>${capacity}</b><span>kapacita</span></div><div><b>${usable}</b><span>použitelných</span></div><div><b>${plan.seats.filter(s=>s.locked).length}</b><span>uzamčeno</span></div></div><div class="seating-action-stack"><button class="primary-button compact" data-action="assign-seating">Rozsadit třídu</button><button class="secondary-button compact" data-action="rotate-seating">Rotovat místa</button><button class="small-button" data-action="clear-seating">Vymazat obsazení</button></div><div class="seating-output-actions"><button class="small-button projection-launch" data-action="project-seating">Promítnout</button><button class="small-button" data-action="download-seating-pdf">Stáhnout PDF · A4 naležato</button><button class="small-button" data-action="print-seating">Tisk · A4 naležato</button></div><div class="front-preference-list"><span>POTŘEBUJE SEDĚT VPŘEDU</span>${students.map(student=>`<label><input type="checkbox" data-action="front-preference" data-id="${student.id}" ${student.frontPreference?'checked':''}><span>${escapeHtml(student.displayName)}</span></label>`).join('')}</div><div class="rotation-note"><span>↔</span><p>Pravidla <b>„od sebe“</b> ze skupin se použijí také při rozsazení sousedních míst. Ruční přesun poté zůstane zachován i v projekci a tisku.</p></div></aside><section class="seating-stage"><div class="board"><span>TABULE · PŘEDNÍ ČÁST</span></div>${plan.seats.length?renderSeatMap(plan,classItem):`<article class="empty-groups compact-empty"><span>PRÁZDNÁ UČEBNA</span><h2>Nastavte rozložení</h2><p>Vyberte typ učebny a počet míst.</p><button class="primary-button" data-action="apply-seating-layout">Vytvořit učebnu</button></article>`}<div class="seating-drag-help">Obsazené místo můžete přetáhnout na jiné volné nebo obsazené místo. Uzamčená místa zůstávají pevná.</div><div class="seating-legend"><span><i class="seat-dot occupied"></i>obsazeno</span><span><i class="seat-dot locked"></i>uzamčeno</span><span><i class="seat-dot blocked"></i>mimo provoz</span></div></section></div>`;bindSeatingTemplateLocal()}
...
function updateSeatingTemplateCopy(){const select=$('#seatingTemplate'),rows=$('#seatingRows'),columns=$('#seatingColumns');if(!select||!rows||!columns)return;const copy=seatingTemplateCopy(select.value);$('#seatRowsLabel').textContent=copy.rows;$('#seatColumnsLabel').textContent=copy.columns;$('#seatingTemplateHelp').textContent=copy.help;$('#seatingCapacityNote').textContent=seatingCapacityText(select.value,rows.value,columns.value)}
function bindSeatingTemplateLocal(){const select=$('#seatingTemplate'),rows=$('#seatingRows'),columns=$('#seatingColumns');if(!select)return;select.addEventListener('change',updateSeatingTemplateCopy);rows?.addEventListener('input',updateSeatingTemplateCopy);columns?.addEventListener('input',updateSeatingTemplateCopy)}
function clearSeatingDragHighlight(){$$('.seat.drag-over').forEach(seat=>seat.classList.remove('drag-over'))}
function bindSeatingUi(){document.addEventListener('change',event=>{const action=event.target.dataset.action;if(action==='front-preference')updateStudent(getSelectedClass().id,event.target.dataset.id,{frontPreference:event.target.checked});if(action==='seat-student'){if(!setSeatStudent(event.target.dataset.id,event.target.value))toast('Toto místo je uzamčené nebo nedostupné.','info');renderSeatingView()}});document.addEventListener('dragstart',event=>{const seat=event.target.closest('.seat[draggable="true"]');if(!seat||!seat.closest('#seatingWorkspace')||event.target.closest('select,button,input')){if(seat)event.preventDefault();return}App.ui.seatingDragStudentId=seat.dataset.studentId||'';App.ui.seatingDragSeatId=seat.dataset.seatId||'';seat.classList.add('dragging');if(event.dataTransfer){event.dataTransfer.effectAllowed='move';event.dataTransfer.setData('text/plain',App.ui.seatingDragStudentId)}});document.addEventListener('dragover',event=>{const seat=event.target.closest('.seat');if(!seat||!seat.closest('#seatingWorkspace')||!App.ui.seatingDragStudentId)return;const model=getSelectedClass()?.seatingPlan?.seats.find(item=>item.id===seat.dataset.seatId);if(!model||model.blocked||model.locked)return;event.preventDefault();clearSeatingDragHighlight();seat.classList.add('drag-over');if(event.dataTransfer)event.dataTransfer.dropEffect='move'});document.addEventListener('drop',event=>{const seat=event.target.closest('.seat');if(!seat||!seat.closest('#seatingWorkspace')||!App.ui.seatingDragStudentId)return;event.preventDefault();const moved=setSeatStudent(seat.dataset.seatId,App.ui.seatingDragStudentId);App.ui.seatingDragStudentId='';App.ui.seatingDragSeatId='';clearSeatingDragHighlight();if(moved){renderSeatingView();toast('Student byl přesunut.','success')}else toast('Na toto místo nelze studenta přesunout.','info')});document.addEventListener('dragend',event=>{event.target.closest('.seat')?.classList.remove('dragging');App.ui.seatingDragStudentId='';App.ui.seatingDragSeatId='';clearSeatingDragHighlight()});document.addEventListener('click',event=>{const button=event.target.closest('[data-action]');if(!button)return;const action=button.dataset.action;if(action==='project-seating'){openProjection('seating');return}if(action==='download-seating-pdf'){downloadSeatingPdf().catch(error=>toast(error.message,'error'));return}if(action==='print-seating'){try{printSortioDocument('seating')}catch(error){toast(error.message,'error')}return}if(!['apply-seating-layout','assign-seating','rotate-seating','clear-seating','toggle-seat-block','toggle-seat-lock'].includes(action))return;try{if(action==='apply-seating-layout')configureSeating({template:$('#seatingTemplate')?.value||'rows',rows:Number($('#seatingRows')?.value||4),columns:Number($('#seatingColumns')?.value||6)});if(action==='assign-seating'){assignSeating();toast('Zasedací pořádek byl vytvořen.','success')}if(action==='rotate-seating'){if(!rotateSeating())throw new Error('Pro rotaci nejsou alespoň dva obsazení studenti.');toast('Místa byla rotována.','success')}if(action==='clear-seating')clearSeatingAssignments();if(action==='toggle-seat-block')toggleSeatBlocked(button.dataset.id);if(action==='toggle-seat-lock'){if(!toggleSeatLock(button.dataset.id))throw new Error('Uzamknout lze pouze obsazené místo.')}renderSeatingView()}catch(error){toast(error.message,'error')}})}
...
;
function printDocumentShell(title,body,{orientation='portrait',documentClass=''}={}){const classItem=getSelectedClass();const logoUrl=new URL('./assets/brand/school-logo.png',location.href).href;const dateLabel=new Intl.DateTimeFormat('cs-CZ',{dateStyle:'long'}).format(new Date());const pageSize=orientation==='landscape'?'A4 landscape':'A4 portrait';const html=`<!doctype html><html lang="cs"><head><meta charset="utf-8"><title>${escapeHtml(title)} · ${escapeHtml(classItem?.name||'SORTIO')}</title><style>@page{size:${pageSize};margin:9mm}*{box-sizing:border-box}html,body{margin:0;padding:0}body{font-family:Arial,"Helvetica Neue",sans-serif;color:#172033;background:#fff;-webkit-print-color-adjust:exact;print-color-adjust:exact}.print-page{min-height:100%;display:flex;flex-direction:column}.print-header{display:grid;grid-template-columns:auto 1fr auto;gap:11mm;align-items:center;border-bottom:1.6px solid #172033;padding:0 0 5mm;margin-bottom:5mm}.school-brand{display:flex;align-items:center;gap:4mm}.school-brand img{display:block;width:22mm;height:22mm;object-fit:contain}.school-brand div{display:grid;gap:1mm}.school-brand strong{font-size:10pt;line-height:1.12}.school-brand span{font-size:7.5pt;color:#667085}.doc-heading{text-align:center}.doc-heading h1{font-size:18pt;line-height:1.05;margin:0 0 2mm}.doc-heading p{margin:0;font-size:9pt;font-weight:700}.print-date{text-align:right;font-size:8pt;color:#5b6575;white-space:nowrap}.grid{display:grid;grid-template-columns:1fr 1fr;gap:4mm}.card{break-inside:avoid;border:1px solid #cbd2dc;border-radius:3mm;padding:3.5mm}.card h2{font-size:11pt;margin:0 0 2mm}.card ul{padding-left:5mm;margin:0}.card li{margin:1.3mm 0;font-size:8.5pt}.meta{font-size:7.5pt;color:#6c7480}.roles{margin-top:2mm;border-top:1px solid #e0e4ea;padding-top:2mm;font-size:7.5pt}.board{width:70%;margin:0 auto 4mm;padding:2mm;border:1.6px solid #172033;border-radius:1.5mm;text-align:center;font-size:8pt;font-weight:800;letter-spacing:.08em}.seat-grid{display:grid;gap:1.5mm}.seat{min-height:12mm;border:1px solid #bfc7d2;border-radius:1.5mm;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:1.2mm;font-size:8pt;line-height:1.15;overflow:hidden}.seat small{font-size:5.7pt;color:#7a8390;line-height:1}.seat b{font-size:var(--seat-font,8pt);line-height:1.05;overflow-wrap:anywhere}.seat.blocked{border-style:dashed;color:#9aa1aa;background:#f5f6f7}.seating-sheet{display:flex;flex-direction:column;min-height:0;flex:1}.seating-meta{display:flex;justify-content:space-between;align-items:center;gap:6mm;margin:-1mm 0 3mm;font-size:7.4pt;color:#5b6575}.seating-sheet .seat-grid{flex:1;grid-auto-rows:minmax(10mm,1fr);align-content:stretch}.stats{width:100%;border-collapse:collapse}.stats th,.stats td{border-bottom:1px solid #d7dce3;padding:2mm;text-align:left;font-size:8pt}.name-cards{display:grid;grid-template-columns:repeat(3,1fr);gap:3mm}.name-card{height:18mm;border:1px dashed #8c96a5;display:grid;place-items:center;text-align:center;font-size:11pt;font-weight:bold;break-inside:avoid}.print-footer{margin-top:5mm;border-top:1px solid #d4d8df;padding-top:2.5mm;display:flex;justify-content:space-between;gap:5mm;font-size:6.7pt;color:#6b7480}.print-footer b{color:#172033}@media print{body{width:auto}.print-page{page-break-after:avoid}}</style></head><body><main class="print-page ${escapeHtml(documentClass)}"><header class="print-header"><div class="school-brand"><img src="${escapeHtml(logoUrl)}" alt="Logo školy"><div><strong>Gymnázium, Ostrava-Hrabůvka</strong><span>příspěvková organizace</span></div></div><div class="doc-heading"><h1>${escapeHtml(title)}</h1><p>${escapeHtml(classItem?.name||'SORTIO')}${classItem?.schoolYear?` · ${escapeHtml(classItem.schoolYear)}`:''}</p></div><div class="print-date">${escapeHtml(dateLabel)}</div></header>${body}<footer class="print-footer"><span><b>SORTIO</b> · organizátor třídy</span><span>Gymnázium, Ostrava-Hrabůvka</span></footer></main><script>window.addEventListener('load',()=>{const logo=document.querySelector('.school-brand img');const printNow=()=>setTimeout(()=>window.print(),80);if(logo?.complete&&logo.naturalWidth){printNow();return}logo?.addEventListener('load',printNow,{once:true});logo?.addEventListener('error',()=>{document.body.innerHTML='<main style="font:16px Arial;padding:32px"><h1>Tisk nebyl spuštěn</h1><p>Nepodařilo se načíst povinné logo školy. Zavřete okno a zkuste tisk znovu.</p></main>'},{once:true})});<\/script></body></html>`;const win=window.open('','_blank');if(!win){toast('Prohlížeč zablokoval tiskové okno. Povolte vyskakovací okna.','error');return}win.document.write(html);win.document.close();App.ui.printWindows=(App.ui.printWindows||[]).filter(item=>item&&!item.closed);App.ui.printWindows.push(win);recordEvent('print_export',{title,orientation})}
function printSortioDocument(type){const classItem=getSelectedClass();if(!classItem)throw new Error('Nejprve vyberte třídu.');if(type==='groups'){if(!classItem.currentGroups.length)throw new Error('Nejprve vytvořte skupiny.');const body=`<div class="grid">${classItem.currentGroups.map(group=>`<section class="card"><h2>${escapeHtml(group.name)} ${group.topic?`· ${escapeHtml(group.topic)}`:''}</h2><ul>${resolveStudents(group.studentIds,classItem).map(student=>`<li>${escapeHtml(student.displayName)}${student.id===group.spokespersonId?' – mluvčí':''}</li>`).join('')}</ul>${Object.keys(group.roleAssignments||{}).length?`<div class="roles">${Object.entries(group.roleAssignments).map(([role,id])=>`<b>${escapeHtml(role)}:</b> ${escapeHtml(classItem.students.find(s=>s.id===id)?.displayName||'—')}`).join('<br>')}</div>`:''}</section>`).join('')}</div>`;printDocumentShell('Skupiny, role a témata',body);return}if(type==='seating'){const plan=classItem.seatingPlan,seats=plan.seats||[];if(!seats.length)throw new Error('Nejprve nastavte rozložení učebny.');const columns=Math.max(...seats.map(s=>s.column),0)+1;const fontSize=columns>=20?6.2:columns>=15?6.8:columns>=11?7.4:8.2;const body=`<section class="seating-sheet"><div class="seating-meta"><span>${escapeHtml(seatingCapacityText(plan.template,plan.rows,plan.columns))}</span><span>Tabule je nahoře · ruční změny jsou zahrnuty</span></div><div class="board">TABULE · PŘEDNÍ ČÁST TŘÍDY</div><div class="seat-grid" style="grid-template-columns:repeat(${columns},minmax(0,1fr));--seat-font:${fontSize}pt">${seats.map(seat=>{const student=classItem.students.find(s=>s.id===seat.studentId);return`<div class="seat ${seat.blocked?'blocked':''}" style="grid-row:${seat.row+1};grid-column:${seat.column+1}"><small>${escapeHtml(seat.label)}</small><b>${seat.blocked?'Mimo provoz':escapeHtml(student?.displayName||'Volné místo')}</b></div>`}).join('')}</div></section>`;printDocumentShell('Zasedací pořádek',body,{orientation:'landscape',documentClass:'seating-print'});return}if(type==='engagement'){const rows=engagementStats(classItem).map(item=>`<tr><td>${escapeHtml(item.student.displayName)}</td><td>${item.count}</td><td>${item.lastAt?formatDateTime(item.lastAt):'—'}</td></tr>`).join('');printDocumentShell('Přehled zapojení',`<table class="stats"><thead><tr><th>Student</th><th>Počet zapojení</th><th>Naposledy</th></tr></thead><tbody>${rows}</tbody></table>`);return}if(type==='cards'){printDocumentShell('Kartičky se jmény',`<div class="name-cards">${classStudents(classItem).map(student=>`<div class="name-card">${escapeHtml(student.displayName)}</div>`).join('')}</div>`);return}}

function seatingPdfFileName(classItem){const clean=String(classItem?.name||'trida').normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-zA-Z0-9_-]+/g,'-').replace(/^-+|-+$/g,'')||'trida';return`SORTIO-zasedaci-poradek-${clean}.pdf`}
function loadRequiredSchoolLogo(){return new Promise((resolve,reject)=>{const image=new Image();image.decoding='async';image.onload=()=>image.naturalWidth?resolve(image):reject(new Error('Povinné logo školy se nepodařilo načíst.'));image.onerror=()=>reject(new Error('Povinné logo školy se nepodařilo načíst.'));image.src=new URL('./assets/brand/school-logo.png',location.href).href})}
function canvasRoundedRect(ctx,x,y,w,h,r=12){const radius=Math.min(r,w/2,h/2);ctx.beginPath();ctx.moveTo(x+radius,y);ctx.arcTo(x+w,y,x+w,y+h,radius);ctx.arcTo(x+w,y+h,x,y+h,radius);ctx.arcTo(x,y+h,x,y,radius);ctx.arcTo(x,y,x+w,y,radius);ctx.closePath()}
function canvasFitText(ctx,text,maxWidth,startSize,minSize=14,weight=700){let size=startSize;do{ctx.font=`${weight} ${size}px Arial, sans-serif`;if(ctx.measureText(text).width<=maxWidth)return size;size-=1}while(size>minSize);return Math.max(size,minSize)}
function canvasSeatNameLines(name){const words=String(name||'').trim().split(/\s+/).filter(Boolean);if(words.length<=1)return words.length?words:['Volné místo'];let best=[words[0],words.slice(1).join(' ')];let bestDelta=Math.abs(best[0].length-best[1].length);for(let i=1;i<words.length;i++){const candidate=[words.slice(0,i).join(' '),words.slice(i).join(' ')],delta=Math.abs(candidate[0].length-candidate[1].length);if(delta<bestDelta){best=candidate;bestDelta=delta}}return best}
function jpegDataUrlBytes(dataUrl){const binary=atob(dataUrl.split(',')[1]||'');const bytes=new Uint8Array(binary.length);for(let i=0;i<binary.length;i++)bytes[i]=binary.charCodeAt(i);return bytes}
function concatPdfBytes(parts){const size=parts.reduce((sum,part)=>sum+part.length,0);const result=new Uint8Array(size);let offset=0;for(const part of parts){result.set(part,offset);offset+=part.length}return result}
function imagePdfBlob(jpegBytes,width,height){const enc=new TextEncoder(),parts=[],offsets=[0];let length=0;const push=value=>{const bytes=typeof value==='string'?enc.encode(value):value;parts.push(bytes);length+=bytes.length};push('%PDF-1.4\n%SORTIO\n');const obj=(id,content)=>{offsets[id]=length;push(`${id} 0 obj\n${content}\nendobj\n`)};obj(1,'<< /Type /Catalog /Pages 2 0 R >>');obj(2,'<< /Type /Pages /Kids [3 0 R] /Count 1 >>');obj(3,'<< /Type /Page /Parent 2 0 R /MediaBox [0 0 841.89 595.28] /Resources << /XObject << /Im0 4 0 R >> >> /Contents 5 0 R >>');offsets[4]=length;push(`4 0 obj\n<< /Type /XObject /Subtype /Image /Width ${width} /Height ${height} /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length ${jpegBytes.length} >>\nstream\n`);push(jpegBytes);push('\nendstream\nendobj\n');const commands='q\n841.89 0 0 595.28 0 0 cm\n/Im0 Do\nQ\n';obj(5,`<< /Length ${enc.encode(commands).length} >>\nstream\n${commands}endstream`);const xref=length;push('xref\n0 6\n0000000000 65535 f \n');for(let i=1;i<=5;i++)push(`${String(offsets[i]).padStart(10,'0')} 00000 n \n`);push(`trailer\n<< /Size 6 /Root 1 0 R >>\nstartxref\n${xref}\n%%EOF`);return new Blob([concatPdfBytes(parts)],{type:'application/pdf'})}
function downloadBlobFile(blob,fileName){const url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download=fileName;link.rel='noopener';document.body.appendChild(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),1200)}
async function downloadSeatingPdf(){const classItem=getSelectedClass();if(!classItem)throw new Error('Nejprve vyberte třídu.');const plan=classItem.seatingPlan,seats=plan?.seats||[];if(!seats.length)throw new Error('Nejprve nastavte rozložení učebny.');const logo=await loadRequiredSchoolLogo();const width=2376,height=1680,pixelWidth=3564,pixelHeight=2520,canvas=document.createElement('canvas');canvas.width=pixelWidth;canvas.height=pixelHeight;const ctx=canvas.getContext('2d',{alpha:false});if(!ctx)throw new Error('PDF se nepodařilo připravit.');ctx.scale(pixelWidth/width,pixelHeight/height);ctx.fillStyle='#ffffff';ctx.fillRect(0,0,width,height);const left=86,right=86,top=70;ctx.drawImage(logo,left,top,116,116);ctx.fillStyle='#172033';ctx.font='700 30px Arial, sans-serif';ctx.fillText('Gymnázium, Ostrava-Hrabůvka',224,top+40);ctx.fillStyle='#667085';ctx.font='22px Arial, sans-serif';ctx.fillText('příspěvková organizace',224,top+76);ctx.textAlign='center';ctx.fillStyle='#172033';ctx.font='700 52px Arial, sans-serif';ctx.fillText('Zasedací pořádek',width/2,top+48);ctx.font='700 28px Arial, sans-serif';ctx.fillText(`${classItem.name||'SORTIO'}${classItem.schoolYear?` · ${classItem.schoolYear}`:''}`,width/2,top+91);ctx.textAlign='right';ctx.fillStyle='#667085';ctx.font='22px Arial, sans-serif';ctx.fillText(new Intl.DateTimeFormat('cs-CZ',{dateStyle:'long'}).format(new Date()),width-right,top+48);ctx.strokeStyle='#172033';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(left,top+142);ctx.lineTo(width-right,top+142);ctx.stroke();ctx.textAlign='left';ctx.fillStyle='#5b6575';ctx.font='21px Arial, sans-serif';ctx.fillText(seatingCapacityText(plan.template,plan.rows,plan.columns),left,top+191);ctx.textAlign='right';ctx.fillText('Tabule je nahoře · ruční změny jsou zahrnuty',width-right,top+191);const boardY=top+222,boardW=Math.round((width-left-right)*.68),boardX=Math.round((width-boardW)/2),boardH=66;ctx.fillStyle='#f7f8fa';canvasRoundedRect(ctx,boardX,boardY,boardW,boardH,14);ctx.fill();ctx.strokeStyle='#172033';ctx.lineWidth=3;ctx.stroke();ctx.textAlign='center';ctx.fillStyle='#172033';ctx.font='800 24px Arial, sans-serif';ctx.fillText('TABULE · PŘEDNÍ ČÁST TŘÍDY',width/2,boardY+42);const maxColumn=Math.max(...seats.map(seat=>seat.column),0)+1,maxRow=Math.max(...seats.map(seat=>seat.row),0)+1;const gridX=left,gridY=boardY+100,gridW=width-left-right,footerY=height-86,gridH=footerY-gridY-34,gap=Math.max(8,Math.min(18,gridW/maxColumn*.055)),cellW=(gridW-gap*(maxColumn-1))/maxColumn,cellH=(gridH-gap*(maxRow-1))/maxRow;for(const seat of seats){const x=gridX+seat.column*(cellW+gap),y=gridY+seat.row*(cellH+gap),student=classItem.students.find(item=>item.id===seat.studentId),label=seat.blocked?'Mimo provoz':student?.displayName||'Volné místo';ctx.save();if(seat.blocked)ctx.setLineDash([10,7]);ctx.fillStyle=seat.blocked?'#f3f4f6':student?'#ffffff':'#fafbfc';ctx.strokeStyle=seat.blocked?'#a8afb9':'#bfc7d2';ctx.lineWidth=2;canvasRoundedRect(ctx,x,y,cellW,cellH,10);ctx.fill();ctx.stroke();ctx.setLineDash([]);ctx.textAlign='center';ctx.fillStyle='#7a8390';ctx.font=`${Math.max(12,Math.min(18,cellH*.12))}px Arial, sans-serif`;ctx.fillText(seat.label,x+cellW/2,y+Math.max(18,cellH*.19));const lines=canvasSeatNameLines(label),available=cellW-20,start=Math.max(17,Math.min(31,cellH*.25));let font=start;for(const line of lines)font=Math.min(font,canvasFitText(ctx,line,available,start,13,700));ctx.font=`700 ${font}px Arial, sans-serif`;ctx.fillStyle=seat.blocked?'#8b929c':'#172033';const lineHeight=font*1.08,base=y+cellH*.58-(lines.length-1)*lineHeight/2;lines.slice(0,2).forEach((line,index)=>ctx.fillText(line,x+cellW/2,base+index*lineHeight));ctx.restore()}ctx.strokeStyle='#d4d8df';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(left,footerY);ctx.lineTo(width-right,footerY);ctx.stroke();ctx.textAlign='left';ctx.fillStyle='#172033';ctx.font='700 20px Arial, sans-serif';ctx.fillText('SORTIO',left,footerY+40);ctx.fillStyle='#6b7480';ctx.font='19px Arial, sans-serif';ctx.fillText('· organizátor třídy',left+78,footerY+40);ctx.textAlign='right';ctx.fillText('Gymnázium, Ostrava-Hrabůvka',width-right,footerY+40);const jpeg=jpegDataUrlBytes(canvas.toDataURL('image/jpeg',.94));downloadBlobFile(imagePdfBlob(jpeg,pixelWidth,pixelHeight),seatingPdfFileName(classItem));recordEvent('pdf_export',{type:'seating',orientation:'landscape'});toast('PDF zasedacího pořádku bylo připraveno ke stažení.','success')}
...
function lessonBoardFullscreenStudio(){const studio=document.fullscreenElement;return studio?.matches?.('.lesson-board-studio')?studio:null}
function lessonBoardRefreshInPlace(studio=lessonBoardFullscreenStudio()){if(!studio)return false;const holder=document.createElement('div');holder.innerHTML=lessonBoardPanelHtml();const next=holder.querySelector('.lesson-board-studio');if(!next)return false;studio.innerHTML=next.innerHTML;lessonBoardAfterRender();return true}
function lessonBoardRefreshView(){const studio=lessonBoardFullscreenStudio();if(studio)return lessonBoardRefreshInPlace(studio);renderToolsView();return true}
function lessonBoardPersist(event='lesson_board_update',{render=true}={}){const board=lessonBoardState();board.updatedAt=nowIso();const fullscreen=render?lessonBoardFullscreenStudio():null;saveData({event,render:render&&!fullscreen});if(render&&fullscreen)lessonBoardRefreshInPlace(fullscreen)}
