...
function lessonBoardBackgroundImage(scene){const bg=scene?.background||{},url=bg.type==='image'?sanitizeLessonImageUrl(bg.url):'';return url?`<img class="lesson-board-background-image" src="${escapeHtml(url)}" alt="" aria-hidden="true">`:''}
...
function lessonBoardConfirmClear(target){if(!App.settings.confirmDestructive)return true;if(!lessonBoardFullscreenRoot())return confirm('Odstranit všechny widgety z pracovní plochy? Pozadí zůstane zachováno.');const now=Date.now();if(lessonClearArmedUntil>now){lessonClearArmedUntil=0;return true}lessonClearArmedUntil=now+4000;if(target){target.textContent='Opravdu vyčistit?';target.classList.add('danger-armed');setTimeout(()=>{if(Date.now()>=lessonClearArmedUntil&&target.isConnected){target.textContent='Vyčistit panel';target.classList.remove('danger-armed')}},4100)}return false}
...
function lessonBoardWireImageErrors(){$$('.board-image-wrap img,.lesson-board-background-image').forEach(image=>{if(image.dataset.errorBound)return;image.dataset.errorBound='1';image.addEventListener('error',()=>{image.hidden=true;const fallback=image.parentElement?.querySelector('.image-load-error');if(fallback)fallback.hidden=false},{once:true})})}
...
function renderProductionHealth(){const root=$('#productionHealth');if(!root)return;const health=storageHealthSnapshot(),invalidPrimary=health.primaryPresent&&!health.primaryValid,invalidLastGood=health.lastGoodPresent&&!health.lastGoodValid;const status=!health.available?'bad':invalidPrimary||invalidLastGood||!!App.storageError?'warn':'good';const healthMark=(present,valid)=>present?(valid?'✓':'!'):'—';root.innerHTML=`<div class="health-head"><div><span>STAV DATOVÉHO TREZORU</span><h3>${status==='good'?'Připraveno k provozu':status==='warn'?'Vyžaduje pozornost':'Úložiště není dostupné'}</h3></div><i class="health-dot ${status}"></i></div><div class="health-grid"><span><b>${healthMark(health.primaryPresent,health.primaryValid)}</b> Primární data</span><span><b>${healthMark(health.lastGoodPresent,health.lastGoodValid)}</b> Bezpečná kopie</span><span><b>${health.recoveryAvailable?'✓':'—'}</b> Stav před importem</span><span><b>${health.saveCount}</b> Bezpečných zápisů</span></div><small>${health.lastSavedAt?`Poslední zápis: ${formatDateTime(health.lastSavedAt)} · ${Math.ceil(health.primaryBytes/1024)} kB`:'Úložiště je připravené. Data zatím nebyla uložena.'}</small>${App.recoveryState?.message?`<p class="recovery-notice">${escapeHtml(App.recoveryState.message)}</p>`:''}`;const restore=$('#restoreRecovery');if(restore)restore.disabled=!health.recoveryAvailable}
