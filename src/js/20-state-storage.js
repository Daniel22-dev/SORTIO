...
function safeStorage(){try{const s=window.localStorage;s.setItem('__sortio_test__','1');s.removeItem('__sortio_test__');return s}catch(_){return null}}
...
function saveSettings(){const storage=safeStorage();if(storage)storage.setItem(SETTINGS_KEY,JSON.stringify({...App.settings,lastRoute:App.route}))}
...
function persistDataSnapshot(data,{updateIntegrity=true}={}){const storage=safeStorage();if(!storage)return false;if(updateIntegrity){data.integrity={...(data.integrity||{}),saveCount:Number(data.integrity?.saveCount||0)+1,lastSavedAt:nowIso()}}const text=JSON.stringify(data);storage.setItem(DATA_KEY,text);storage.setItem(LAST_GOOD_KEY,text);return true}
function loadData(){
...
function sanitizeData(raw){const data={...defaultData(),...(raw&&typeof raw==='object'?raw:{}),schema:'sortio-data-v5',version:5};data.classes=Array.isArray(raw?.classes)?raw.classes.map(sanitizeClass).filter(Boolean):[];data.aliases=raw?.aliases&&typeof raw.aliases==='object'?Object.fromEntries(Object.entries(raw.aliases).slice(0,500).map(([key,value])=>[String(key).slice(0,100),String(value).slice(0,100)])):{};data.integrity={saveCount:Math.max(0,Number(raw?.integrity?.saveCount)||0),lastSavedAt:raw?.integrity?.lastSavedAt||raw?.updatedAt||null};if(!data.classes.some(item=>item.id===data.selectedClassId))data.selectedClassId=data.classes.find(item=>!item.archived)?.id||data.classes[0]?.id||null;return data}
function sanitizeClass(item){if(!item||typeof item!=='object')return null;const students=Array.isArray(item.students)?item.students.slice(0,500).map(sanitizeStudent).filter(Boolean):[];const ids=new Set(students.map(student=>student.id));const rules=item.groupRules&&typeof item.groupRules==='object'?item.groupRules:{};return{id:String(item.id||uid('class')),name:String(item.name||'Třída bez názvu').slice(0,160),schoolYear:String(item.schoolYear||'').slice(0,40),archived:!!item.archived,demo:!!item.demo,createdAt:item.createdAt||nowIso(),updatedAt:item.updatedAt||nowIso(),students,drawState:{remainingIds:Array.isArray(item.drawState?.remainingIds)?item.drawState.remainingIds.map(String).filter(id=>ids.has(id)):[],cycle:Number(item.drawState?.cycle||0),lastDraw:item.drawState?.lastDraw||null},drawHistory:Array.isArray(item.drawHistory)?item.drawHistory.slice(0,150):[],engagementHistory:sanitizeEngagementHistory(item.engagementHistory,ids),currentGroups:Array.isArray(item.currentGroups)?item.currentGroups.map(group=>sanitizeGroup(group,ids)).filter(Boolean):[],groupHistory:Array.isArray(item.groupHistory)?item.groupHistory.slice(0,80):[],lastGroupConfig:item.lastGroupConfig&&typeof item.lastGroupConfig==='object'?item.lastGroupConfig:null,groupRules:{together:sanitizePairs(rules.together,ids),apart:sanitizePairs(rules.apart,ids),pins:sanitizePins(rules.pins,ids)},roleCatalog:Array.isArray(item.roleCatalog)&&item.roleCatalog.length?uniqueStrings(item.roleCatalog):defaultRoleCatalog(),topicCatalog:Array.isArray(item.topicCatalog)?uniqueStrings(item.topicCatalog):[],roleHistory:Array.isArray(item.roleHistory)?item.roleHistory.slice(0,500):[],seatingPlan:sanitizeSeatingPlan(item.seatingPlan,ids),toolState:sanitizeToolState(item.toolState)}}
...
function saveData({render=true,event='data_change'}={}){if(!App.data)App.data=defaultData();App.data.updatedAt=nowIso();try{persistDataSnapshot(App.data);App.storageError=null}catch(error){App.storageError={message:error.message,createdAt:nowIso()};captureError(error,'save-data')}App.lastOperation=event;if(render)document.dispatchEvent(new CustomEvent('sortio:data-changed',{detail:{event}}))}
function clearSettings(){safeStorage()?.removeItem(SETTINGS_KEY)}
function clearAllData(){const storage=safeStorage();[DATA_KEY,LAST_GOOD_KEY,RECOVERY_KEY,CORRUPT_KEY,...LEGACY_DATA_KEYS].forEach(key=>storage?.removeItem(key));App.data=defaultData();App.recoveryState={storageAvailable:!!storage,recovered:false,source:'cleared'};saveData({event:'clear_all_data'})}
...
function validateBackupPayload(payload){if(!payload||typeof payload!=='object'||!['sortio-backup-v1','sortio-backup-v2','sortio-backup-v3','sortio-backup-v4'].includes(payload.schema)||!payload.data)throw new Error('Soubor není platná záloha SORTIO.');if(payload.integrity?.checksum){const actual=checksumText(JSON.stringify(sanitizeData(payload.data)));if(actual!==payload.integrity.checksum)throw new Error('Kontrolní součet zálohy nesouhlasí. Soubor mohl být poškozen.')}return sanitizeData(payload.data)}
async function importBackup(file){if(!file)throw new Error('Nebyl vybrán žádný soubor.');if(file.size>MAX_BACKUP_BYTES)throw new Error('Záloha je příliš velká. Maximální podporovaná velikost je 5 MB.');let payload;try{payload=JSON.parse(await file.text())}catch(_){throw new Error('Soubor není platný JSON.')}const incoming=validateBackupPayload(payload);const summary={classes:incoming.classes.length,students:incoming.classes.reduce((sum,item)=>sum+item.students.length,0)};if(App.settings.confirmDestructive!==false&&!confirm(`Načíst zálohu s ${summary.classes} třídami a ${summary.students} studenty? Aktuální stav bude možné obnovit tlačítkem „Vrátit stav před importem“.`))return false;const storage=safeStorage();if(storage&&App.data)storage.setItem(RECOVERY_KEY,JSON.stringify(App.data));App.data=incoming;saveData({event:'backup_import'});return true}
