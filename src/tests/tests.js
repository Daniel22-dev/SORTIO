...
{id:'core',title:'Verze, manifest a identita',tests:[
 ['manifest',async()=>{const j=await json('../studio-manifest.json');return j.id==='sortio'&&j.version==='1.0.13'?'Manifest SORTIO 1.0.13 je platný.':false}],
 ...
 ['service-worker',async()=>text('../sw.js').then(t=>t.includes('ghrab-sortio-v1.0.13')&&t.includes('CACHE_PREFIXES')&&t.includes('CACHE_PREFIXES.some')&&t.includes('key !== CACHE_NAME')?'Service worker má verzi 1.0.13 a izolaci cache.':false)]
]},
...
function protocol(){return JSON.stringify({schema:'sortio-runtime-tests-v5',version:'1.0.13',createdAt:new Date().toISOString(),userAgent:navigator.userAgent,results},null,2)}
