...
{id:'core',title:'Verze, manifest a identita',tests:[
 ['manifest',async()=>{const j=await json('../studio-manifest.json');return j.id==='sortio'&&j.version==='1.1.3'?'Manifest SORTIO 1.1.3 je platný.':false}],
 ...
 ['service-worker',async()=>text('../sw.js').then(t=>t.includes('ghrab-sortio-v1.1.3')&&t.includes('CACHE_PREFIXES')&&t.includes('CACHE_PREFIXES.some')&&t.includes('key !== CACHE_NAME')?'Service worker má verzi 1.1.3 a izolaci cache.':false)]
]},
...
function protocol(){return JSON.stringify({schema:'sortio-runtime-tests-v5',version:'1.1.3',createdAt:new Date().toISOString(),userAgent:navigator.userAgent,results},null,2)}
