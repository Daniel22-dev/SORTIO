...
const expected = {
  appId: env('GHRAB_LIVE_APP_ID'), version: env('GHRAB_LIVE_VERSION'), sourceCommit: env('GHRAB_LIVE_SOURCE_SHA').toLowerCase(),
  artifactDigest: env('GHRAB_LIVE_ARTIFACT_DIGEST').toLowerCase(), manifestSha256: env('GHRAB_LIVE_MANIFEST_SHA256').toLowerCase(),
};
...
const missing = Object.entries(expected).filter(([, v]) => !v).map(([k]) => k); if (!rawUrl) missing.push('url'); if (missing.length) fail('required-input-missing', { missing });
...
let base; try { base = new URL(rawUrl.endsWith('/') ? rawUrl : `${rawUrl}/`); } catch { fail('unparsable-url'); }
if (base.protocol !== 'https:') fail('insecure-url', { origin: base.origin }); if (base.origin !== allowedOrigin) fail('unexpected-origin', { origin: base.origin, allowedOrigin });
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
async function fetchText(url) { const response = await fetch(url, { redirect: 'error', cache: 'no-store', headers: { 'cache-control': 'no-cache', accept: 'application/json' }, signal: AbortSignal.timeout(requestTimeoutMs) }); if (!response.ok) throw new Error(`HTTP ${response.status}`); return response.text(); }
function evaluate(it, mt) {
  const problems = []; let integrity, manifest;
...
const integrityUrl = new URL('release-integrity.json', base).toString(); const manifestUrl = new URL('studio-manifest.json', base).toString(); const history = [];
for (let attempt = 1; attempt <= attempts; attempt++) {
  let outcome; try { const [it, mt] = await Promise.all([fetchText(integrityUrl), fetchText(manifestUrl)]); outcome = evaluate(it, mt); } catch (error) { outcome = { problems: [`fetch:${error.message}`] }; }
  if (!outcome.problems.length) {
    const v = { schema: 'ghrab-live-release-verification-v1', status: 'PASS', attempt, attempts, appId: expected.appId, version: expected.version, sourceCommit: expected.sourceCommit, artifactDigest: expected.artifactDigest, manifestSha256: expected.manifestSha256, sbomSha256: String(outcome.integrity.sbomSha256 || '').toLowerCase() || null, evidenceManifestSha256: String(outcome.integrity.evidenceManifestSha256 || '').toLowerCase() || null, buildProvenanceSha256: String(outcome.integrity.buildProvenanceSha256 || '').toLowerCase() || null, assuranceMode: outcome.integrity.assuranceMode || null, releaseStage: outcome.integrity.releaseStage || null, garpProfile: outcome.integrity.garpProfile || null, gate: outcome.integrity.gate || null, releaseIntegrityUrl: integrityUrl, deployedUrl: base.toString(), verifiedAt: new Date().toISOString() };
    ...
    console.log(JSON.stringify(v, null, 2)); process.exit(0);
  }
  history.push({ attempt, problems: outcome.problems }); if (attempt < attempts) await sleep(Math.min(baseDelayMs * attempt, maxDelayMs));
}
