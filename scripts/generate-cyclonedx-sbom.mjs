...
const root=process.cwd(), packageMeta=JSON.parse(fs.readFileSync(path.join(root,'package.json'),'utf8'));
const outArg=positional[0]||(deployMode?`security/sbom/sortio-${packageMeta.version}-deployment.cdx.json`:`security/sbom/sortio-${packageMeta.version}.cdx.json`);
...
if(!deployMode){
 const lock=JSON.parse(fs.readFileSync('package-lock.json','utf8')), pkg=packageMeta; const entries=[];
 ...
 if(!fs.existsSync(deployRoot))throw new Error(`Deployment missing: ${deployRoot}`); const pkg=packageMeta;
